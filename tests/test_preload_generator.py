...
# r7: the preload also carries the chain_id_for_strategy map and the preload
# generation as top-level assignments (contract §4 / §2).  These are data, not
# calls, so they join the allowed-prefix set the forbidden-pattern test checks.
ALLOWED_PRELOAD_ASSIGNMENTS = (
    "ORCHESTRA_WHITELIST",
    "ORCHESTRA_CHAIN_ID_FOR_STRATEGY",
    "ORCHESTRA_PRELOAD_GENERATION",
)
...
def render_chain_map_and_generation(
    ...
    # r7: bake ORCHESTRA_CHAIN_ID_FOR_STRATEGY (from the adaptive profile
    # sidecar profiles/<profile>.json, contract §4) and ORCHESTRA_PRELOAD_GENERATION
    # (from manager-state.json) into the preload.  The oracle mirrors the ucode
    # generator: with no sidecar (the default for the golden fixture and any
    ...
    entries = ", ".join(f"[{n}]={lua_quote(by_strategy[n])}" for n in nums)
    return [
        f"ORCHESTRA_CHAIN_ID_FOR_STRATEGY = {{ [{lua_quote(askey)}] = {{ {entries} }} }}",
        f"ORCHESTRA_PRELOAD_GENERATION = {int(generation)}",
    ]
...
class PreloadGeneratorStaticTest(unittest.TestCase):
    ...
    def test_generates_only_allowed_preload_calls(self) -> None:
        ...
        # r7: the preload also bakes the chain_id_for_strategy map and the
        # preload generation (contract §4 / §2).
        self.assertIn("ORCHESTRA_CHAIN_ID_FOR_STRATEGY", self.source)
        self.assertIn("ORCHESTRA_PRELOAD_GENERATION", self.source)
        self.assertIn("chain_id_for_strategy", self.source)
...
class PreloadOracleTest(unittest.TestCase):
    ...
    def test_generated_preload_contains_no_forbidden_patterns(self) -> None:
        seeds = load_seeds(STATE_SRC)
        preload = render_preload(seeds)
        allowed_prefixes = tuple(ALLOWED_PRELOAD_CALLS) + ALLOWED_PRELOAD_ASSIGNMENTS
...
class PreloadGeneratorRuntimeTest(unittest.TestCase):
    ...
    def _run(self, *args: str) -> subprocess.CompletedProcess:
        ...
        # r7: point the manager-state + sidecar search roots at the sandbox so
        # the chain_id_for_strategy lookup does not touch the real /etc or /usr.
