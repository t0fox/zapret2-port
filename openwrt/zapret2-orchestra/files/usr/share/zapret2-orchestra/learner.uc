...
function process_event(learned, blocked, manual, ev, seen_keys) {
	...
	if (etype == 'success' || etype == 'fail') {
		...
		if (etype == 'success') {
			...
			// on the AUTO-locked strategy, remove auto_lock (contract §3).
			// Guards: only when auto-locked, only on locked strategy, never
			// on user-locked. Use delete (not = null) so key is absent in JSON.
